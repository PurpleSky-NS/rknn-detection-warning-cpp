...
{
public:
    PacketDecoder(const PacketPuller &streamer, const cv::Size &size={});
    PacketDecoder(const PacketDecoder&)=delete;
    PacketDecoder(PacketDecoder&&)=delete;
    ~PacketDecoder();

    // 解码一帧并返回内置缓存，注意再次调用后会修改内置缓存，如有并发调用该接口的需求请及时将数据复制出来
    std::shared_ptr<cv::Mat> operator()(std::shared_ptr<AVPacket> pkt);
